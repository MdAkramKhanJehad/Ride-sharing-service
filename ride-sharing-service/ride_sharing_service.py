...
@app.route('/ridesharingservice/driver', methods=['POST'])
def driver():
    data = request.json
    drivers.append(data)
    return data
...
@app.route('/ridesharingservice/rider', methods=['POST'])
def rider():
    data = request.json
    riders.append(data)
    return data
...
def find_a_driver():
    ...
    if len(drivers) > 0 and len(riders) > 0:
        ...
        r = requests.post("http://communication-service:8080/newPair", json=json.dumps(pair))
        riders.pop(0)
